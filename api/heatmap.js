...
function generateWeeksFromPastYear(selectedYear) {
  ...
  // Pad the first week with nulls for days before the start date
  // so array indices align with day-of-week (0=Sun, 1=Mon, etc.)
  const startDayOfWeek = lastYearsDate.getUTCDay();
...
export default async function renderHeatMap(selectedYear) {
  ...
  const endDay = flatDates[flatDates.length - 1];
  ...
  for (const { date, count } of activityRecords) {
    // transform results into a map {"2025-05-05": 5}
  ...
  for (let i = 0; i <= 6; i++) {
    ...
    let string = `<tr>`;
    if (i in DAY_MAP)
      string += `<td class="activity-label"><span>${DAY_MAP[i]}</span></td>`; //generate column labels for the days
    ...
    for (let week of dates) {
      //iterate over each week only one day at a time, so all mondays of the year, then all tuesdays etc.
      // each row of the heatmap contains the same weekday for every cell, row 1 = every monday from 2024-2025
      if (!week[i]) {
        // Empty cell for padding (days before year start or after year end)
